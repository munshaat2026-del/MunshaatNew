...
import { Locale, CareersCreateInput,CareersUpdateInput, CareersGetPayload } from "@/types";
import { revalidateTag, unstable_cache } from "next/cache";
import {UTApi} from "uploadthing/server";

const utapi = new UTApi();

export const addNewCareer = async (data: CareersCreateInput) => {
...
export const updateCareer= async(id:string, data:CareersUpdateInput)=>{
    try {
  ...
  unstable_cache(
    async()=>{
      try {
        ...
        const translatedCareers= result.map((career:CareersGetPayload)=>{
          return {
            id:career.id,
...
export const getCareerBySlugAndLocale= (slug:string, locale:Locale)=> unstable_cache(
  async()=>{
    try {
      const result= await prisma.careers.findUnique({where:{slug}})
      ...
      return {
        data: null,
        message: `There is No Career with ${slug}`,
        status: 200,
      }
  ...
  },[`career-by-slug-and-locale-${slug}-${locale}`],{tags:["careers"], revalidate:3600}
)()
