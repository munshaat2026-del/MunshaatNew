...
import { Locale, ClientsCreateInput, ClientsUpdateInput } from "@/types";
import { revalidateTag, unstable_cache } from "next/cache";
import { UTApi } from "uploadthing/server";

const utapi = new UTApi();

export const addNewClient = async (data: ClientsCreateInput) => {
  try {
  ...
  unstable_cache(
    async () => {
      try {
        ...
        return {
          data: null,
          message: "Error In Adding The Client",
          status: 500,
        };
      }
    },
    [`client-by-id-${clientId}`],
    { tags: ["clients"], revalidate: 3600 },
  )();
...
export const getAllClientsByLocale = (locale: Locale) =>
  unstable_cache(
    async () => {
      try {
        const result = await prisma.clients.findMany({});
        const translatedClients = result.map((client: ClientsCreateInput) => {
          return {
            id: client.id,
