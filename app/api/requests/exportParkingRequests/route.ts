...
import { RequestsGetPayloadParkingNameAndId, type RequestsGetPayloadParking } from "@/types/index";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const allRows: RequestsGetPayloadParkingNameAndId[] = [];
    let page = 1;
    let totalPages = 1;

    do {
      const res =  await getAllParkingrequestsByFilters(page, {
    ...
    const sheet = workbook.addWorksheet("Parking Requests");
    ...
    const safeProgram = (filters.parkingId ?? "all").replace(
      /[^a-z0-9-_]/gi,
      "_",
    );
