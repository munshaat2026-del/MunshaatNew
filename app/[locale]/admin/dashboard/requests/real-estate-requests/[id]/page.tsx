...
import { getRealEstateNameById } from "@/app/server/real_estates/services";
...
async function Page({ params, searchParams }: Props) {
  ...
  const realEstateResult = await getRealEstateNameById(id);

  if (!realEstateResult || !realEstateResult.data) {
    notFound();
  }  
  const realEstateDetails = realEstateResult.data;
  ...
  return (
    <div className="flex flex-col justify-start items-start ml-5 md:ml-7 w-[88vw] md:w-[68vw] xl:w-[80vw]">
      <h1 className="text-2xl font-semibold mb-4 border-b p-1 w-full">
        Requests On {realEstateDetails.name_en}
      </h1>
