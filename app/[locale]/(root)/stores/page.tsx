...
export default async function StoresPage({ params, searchParams }: Props) {
  ...
  // Data Fetching - Explicitly set to "store"
  const stores = await getAllRealEstatesByTypeByLocale(
    "store",
    locale,
    page,
    filter,
  ...
  return (
    <div className="min-h-screen mt-24 bg-slate-50/50 text-slate-900 font-sans pb-20">
      {/* Reusing the Header - You might want to pass a 'title' prop if the component supports it */}
      <StoresHeader primaryColor={primaryColor} locale={locale} />
            ...
            {hasStores ? (
              ...
              <div className="flex flex-col items-center justify-center py-24 px-6 bg-white border-2 border-dashed border-slate-200 rounded-2xl text-center">
                ...
                <p className="text-slate-500 text-sm max-w-xs mt-3 font-medium">
                  {isArabic
                    ? "لم نتمكن من العثور على أي محلات تطابق الفلاتر الحالية. حاول تعديل البحث."
                    : "We couldn't find any stores matching your current filters. Try adjusting your criteria."}
                </p>
