...
export default function Navbar({ isComingSoon }: { isComingSoon: Boolean }) {
  ...
  // Refactored to handle multiple dropdowns dynamically
  ...
  const navLinks = [
    { name: t("home"), href: "/" },
    {
      ...
      isDropdown: true,
      items: [
        { name: t("aboutUs") || "About Us", href: "/about#about-us" },
        {
          name: t("ourComplexes") || "Our Complexes",
          href: "/about#our-complexes",
        },
        { name: t("ourTeam") || "Our Team", href: "/about#our-team" },
        { name: t("ourClients") || "Our Clients", href: "/about#our-clients" },
      ],
    },
    {
      name: t("realEstates"),
                ...
                if (link.isDropdown) {
                  return (
                    <div key={link.name} className="relative group py-2">
                      ...
                      <div className="absolute top-full inset-s-0 w-44 mt-2 bg-white border border-slate-100 shadow-xl opacity-0 translate-y-2 invisible group-hover:opacity-100 group-hover:translate-y-0 group-hover:visible transition-all duration-300 z-50">
        ...
        <div className="p-8 flex flex-col h-full overflow-y-auto">
          ...
          <div className="flex flex-col">
            {navLinks.map((link) => {
              ...
              if (link.isDropdown) {
                const isThisDropdownOpen = openMobileDropdown === link.name;
                ...
                return (
                  <div key={link.name} className="border-b border-slate-50">
                    <button
                      onClick={() =>
                        setOpenMobileDropdown(
                          isThisDropdownOpen ? null : link.name,
                        )
                      }
                    ...
                    >
                      ...
                      <ChevronDown
                        size={18}
                        className={`transition-transform duration-300 opacity-40 ${isThisDropdownOpen ? "rotate-180" : ""}`}
                      />
                    </button>
                    <div
                      className={`flex flex-col overflow-hidden transition-all duration-300 ${isThisDropdownOpen ? "max-h-60 pb-4" : "max-h-0"}`}
                    >
                      {link.items?.map((subItem) => (
                        <Link
                          key={subItem.name}
