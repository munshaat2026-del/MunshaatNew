...
export default function ServicesSection({ primaryColor }: ServicesSectionProps) {
  ...
  return (
    <section className="py-40 px-6 md:px-20 bg-white">
      ...
      <div className={`max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-0 border-t ${isAr ? 'border-r' : 'border-l'} border-slate-100`}>
        {data.items.map((item, i) => {
          ...
          return (
            ...
            >
              ...
              <h3 className={`text-lg font-black uppercase tracking-tight mb-4 transition-transform duration-500 ${isAr ? 'group-hover:-translate-x-2' : 'group-hover:translate-x-2'}`}>
                {item.t}
              </h3>
