...
export default function VideoSection({
  ...
  useEffect(() => {
    if (videoUrl) {
      ReactDOM.preload(videoUrl, { as: "video", fetchPriority: "high" });
    }
  }, [videoUrl]);
  ...
  return (
    <section className="relative w-full h-[80vh] md:h-screen flex items-center justify-center overflow-hidden bg-slate-900">
      {videoUrl && (
        <video
          ref={videoRef}
          key={videoUrl}
      ...
      <div className="absolute inset-0 bg-linear-to-b from-black/30 via-black/50 to-black/80 z-10" />

      {videoUrl && (
        <button
          onClick={toggleSound}
          aria-label={soundOn ? "Mute video" : "Unmute video"}
          className={`absolute top-6 right-6 md:top-8 md:right-8 z-30 ${locale === "ar" ? "rotate-180" : ""}  flex h-12 w-12 items-center justify-center rounded-full border border-white/30 bg-black/30 text-white backdrop-blur-md transition-all duration-300 hover:scale-110 hover:bg-[#0c479a]`}
        >
