...
export default function EmptyTenders({ locale }: { locale: Locale }) {
  ...
  return (
    <div className="min-h-[60vh] mt-20 flex flex-col items-center justify-center px-6 py-20" dir={isAr ? "rtl" : "ltr"}>
      ...
      <div className="text-center max-w-lg">
        ...
        <div className="relative inline-block mb-10">
          ...
          <FileX2 size={64} strokeWidth={1} className="relative text-slate-200 mx-auto" />
        </div>
        ...
        <Link 
          href={`/${locale}`}
          className="inline-flex items-center gap-3 bg-slate-900 text-white px-10 py-5 text-[10px] font-bold uppercase tracking-[0.2em] hover:bg-[#0c479a] transition-all"
        >
      ...
      <div className="mt-24 pt-8 border-t border-slate-100 w-full max-w-7xl flex justify-between items-center opacity-30">
        ...
        <div className="flex gap-1">
          {[1,2,3].map(i => <div key={i} className="w-1 h-1 bg-slate-400" />)}
        </div>
      </div>
    </div>
  );
