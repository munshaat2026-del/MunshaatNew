...
export default function ExecutiveQuote({ primaryColor }: ExecutiveQuoteProps) {
  ...
  return (
    <section className="py-40 px-6 md:px-16 bg-white overflow-hidden">
      <div
        className={`max-w-7xl mx-auto relative bg-[#0a0f1a] flex flex-col ${isAr ? "md:flex-row-reverse" : "md:flex-row"} items-stretch`}
      >
        ...
        <div
          className={`w-full md:w-1/3 p-16 flex items-center justify-center border-b md:border-b-0 ${isAr ? "md:border-l" : "md:border-r"} border-white/10 relative overflow-hidden`}
        >
          <div
            className={`absolute ${isAr ? "-right-10" : "-left-10"} bottom-0 select-none pointer-events-none opacity-[0.05]`}
          >
          ...
          <div className="relative z-10 w-24 h-24 border border-white/20 flex items-center justify-center group hover:border-white transition-colors duration-700">
            <Quote
              size={32}
              className={`transition-transform no-flip duration-500 group-hover:scale-110 ${isAr ? "rotate-180" : ""}`}
              style={{ color: primaryColor }}
            />
            <div
              className={`absolute -top-2 ${isAr ? "-right-2" : "-left-2"} w-4 h-4 border-t ${isAr ? "border-r" : "border-l"} border-white/40`}
            ></div>
            <div
              className={`absolute -bottom-2 ${isAr ? "-left-2" : "-right-2"} w-4 h-4 border-b ${isAr ? "border-l" : "border-r"} border-white/40`}
            ></div>
          </div>
        </div>
        ...
        <div className="w-full md:w-2/3 p-12 md:p-24 relative z-10 space-y-16 flex flex-col justify-center text-start">
          ...
          <div
            className={`flex flex-col md:flex-row md:items-end justify-between gap-8 pt-8 border-t border-white/10 ${isAr ? "md:flex-row-reverse" : ""}`}
          >
