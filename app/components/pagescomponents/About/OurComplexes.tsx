...
export default function RoyalOffsetHero({ complexdata }: Props) {
  ...
  const getContainerStyles = () => {
    if (cardCount >= 6)
      return "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 lg:gap-12";
    ...
    <section className="relative min-h-screen w-full bg-white flex flex-col items-center justify-start overflow-hidden border-b border-slate-900 py-32">
      ...
      <div className="relative z-10 w-full max-w-[1500px] px-10 mb-24 space-y-4">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          <p className="max-w-xs text-[11px] font-bold  tracking-tight text-slate-400 leading-relaxed border-l-2 border-slate-100 pl-4">
            {isAr
        ...
        {complexdata.map((item, idx) => (
          ...
          >
            ...
            <div className="relative h-[550px] w-full overflow-hidden bg-slate-900 shadow-2xl border border-slate-100/10">
              ...
              {/* Card Index */}
              <div
                className={`absolute top-0 ${isAr ? "right-0" : "left-0"} bg-[#0c479a] text-white px-5 py-4 z-20`}
              >
              ...
              <div className="absolute inset-0 p-8 flex flex-col justify-end z-10">
                <div className="space-y-5">
                  ...
                  {/* Expandable Info */}
                  <div
                    className={`overflow-hidden transition-all duration-700 ${active === idx || cardCount >= 6 ? "max-h-40 opacity-100 mt-4" : "max-h-0 opacity-0"}`}
                  >
                    <div
                      className={`flex items-start gap-4 mb-6 text-white/50 border-[#0c479a] ${isAr ? "border-r pr-4 text-right" : "border-l pl-4 text-left"} text-[9px] font-bold uppercase `}
                    >
              ...
              <div
                className={`absolute bottom-0 ${isAr ? "right-0" : "left-0"} h-1 bg-[#0c479a] transition-all duration-1000 ${active === idx ? "w-full" : "w-0"}`}
              />
            </div>
          </Link>
        ))}
