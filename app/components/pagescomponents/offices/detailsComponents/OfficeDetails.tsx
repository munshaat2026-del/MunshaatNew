    ...
    >
      ...
      <div className="lg:col-span-8 space-y-16">
        ...
        <section className="space-y-6">
          ...
          <p
            className={`text-slate-500 text-sm font-medium leading-[2.2] max-w-3xl ${isAr ? "border-r-2 pr-8 text-right" : "border-l-2 pl-8 text-left"} border-slate-100`}
          >
            {realEstate.description}
          </p>
        </section>

        {/* 2. Technical Inventory (Floor & Size) */}
            ...
            <div className="flex items-center gap-6 p-6 bg-slate-50 border border-slate-100 group hover:border-[#0c479a] transition-colors">
              ...
              <div>
                ...
                <p className="text-lg font-bold text-slate-900">
                  {realEstate.size_sqm}{" "}
                  <span className="text-xs font-normal">{isAr ?"متر مربع":"sqm"}</span>
                </p>
              </div>
            </div>
        ...
        <section className="space-y-10">
          ...
          <div
            className={`grid grid-cols-2 md:grid-cols-3 gap-0 border-t ${isAr ? "border-r" : "border-l"} border-slate-100`}
          >
            {realEstate.features.map((feature, i) => (
              <div
                key={i}
                className={`flex flex-col gap-4 p-8 ${isAr ? "border-l" : "border-r"} border-b border-slate-100 hover:bg-slate-50 transition-colors`}
              >
      ...
      <div className="lg:col-span-4">
        <div
          className={`bg-[#0a0f1a] px-4 py-14 ${isAr ? "border-r-4" : "border-l-4"} border-[#0c479a] sticky top-32 space-y-10 shadow-2xl relative overflow-hidden`}
        >
          ...
          <div className="relative z-10 space-y-10">
            ...
            {realEstate.location_link && (
              ...
              >
                ...
                <ArrowRight
                  size={14}
                  className={`${isAr ? "rotate-180" : ""} group-hover:translate-x-1 transition-transform`}
                />
              </a>
            )}

           <Link href={`/request-form/${realEstate.id}`}>
            <button  className="w-full bg-[#0c479a] hover:bg-white hover:text-black text-white font-black py-6 text-[10px] uppercase tracking-[0.5em] transition-all flex items-center justify-center gap-4 group">
              ...
              <ArrowRight
                size={16}
                className={`${isAr ? "rotate-180 group-hover:-translate-x-2" : "group-hover:translate-x-2"} transition-transform`}
              />
            </button></Link>
          </div>
        </div>
