...
export default function ApplicationSidebar({
  ...
  const handleDownload = async (url: string, fileName: string) => {
    ...
    try {
      ...
      toast.error(
        "Could not download the file. Please try opening it in a new tab.",
        { id: toastId },
      );
      console.error(error);
    } finally {
    ...
    <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden sticky top-24">
      <div className="relative h-40 bg-slate-100">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-slate-900/60 to-transparent flex items-end p-6">
          <span className="text-white text-[10px] font-black uppercase  bg-white/10 backdrop-blur-md px-3 py-1.5 rounded-lg border border-white/20">
      ...
      <div className="py-8 px-4 md:px-8">
        ...
        {application.cv ? (
          <Button1
            onClick={() =>
              handleDownload(application.cv, application.full_name)
            }
        ...
        <div className="mt-10 space-y-3">
          ...
          <ContactRow
            ...
            onCopy={() => copy(application.email, "Email")}
          />
          <ContactRow
            icon={<Phone size={14} />}
            value={application.phone_number}
            onCopy={() => copy(application.phone_number, "Phone")}
          />
        </div>
      </div>
    </div>
  ...
  return (
    <div className="group flex items-center justify-between p-3.5 bg-slate-50 rounded-2xl border border-transparent hover:border-slate-200 transition-all">
      ...
      {value && (
        <button
          onClick={onCopy}
          className="p-2 text-slate-300 hover:text-[#0c479a] transition-colors"
        >
