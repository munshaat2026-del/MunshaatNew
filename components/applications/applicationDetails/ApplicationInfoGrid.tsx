    ...
    <div className="bg-white rounded-3xl shadow-sm border border-slate-200 py-10 px-4 lg:px-10">
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-8 mb-12 pb-10 border-b border-slate-100">
        ...
        <div className="space-y-4">
          <div className="flex flex-wrap items-center gap-4">
            ...
            <span className="px-3 py-1 bg-slate-900 text-white text-[10px] font-black uppercase  self-center">
              Active
            </span>
          </div>
          ...
          <div className="flex items-center gap-6">
            <p className="text-slate-400 font-bold text-[11px] uppercase tracking-widest flex items-center gap-2">
              <span className="w-2 h-2 bg-[#0c479a]" />{" "}
              {/* Small industrial accent */}
              Candidate ID:{" "}
        ...
        <div className="grid grid-cols-2 md:flex md:flex-col gap-6 md:gap-4 md:items-end border-t md:border-t-0 pt-6 md:pt-0 border-slate-50">
          ...
          <div className="md:text-right group">
            <p className="text-[10px] font-black text-slate-400 uppercase  mb-1 group-hover:text-slate-600 transition-colors">
              Applied Date
            </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-12">
        ...
        <div className="space-y-8">
          ...
          <InfoItem
            ...
            value={application.national_number}
          />
          <InfoItem
            icon={<Calendar />}
            label="Date of Birth"
      ...
      <div className="mt-16 bg-slate-50 rounded-3xl border border-slate-200 py-10 px-3 lg:px-8 flex flex-col items-center text-center">
        ...
        <p className="text-slate-500 text-xs font-bold max-w-sm leading-relaxed mb-8">
          For full security compliance, verify the candidate&apos;s credentials
          by reviewing the original document uploaded to our secure servers.
        </p>
        {application.cv && (
          <a
            href={application.cv}
            target="_blank"
            className="text-[#0c479a] font-black text-xs uppercase tracking-widest hover:text-slate-900 transition-colors flex items-center gap-2"
          >
            Open File in Viewer →
          </a>
        )}
...
function SectionLabel({ label }: { label: string }) {
  return (
    <h4 className="text-[11px] font-black text-[#0c479a] uppercase  pb-2 border-b-2 border-[#0c479a]/10">
      {label}
    </h4>
  );
}
