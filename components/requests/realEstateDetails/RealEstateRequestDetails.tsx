...
export default function RealEstateRequestDetailPage({
  ...
  const { real_estates, ...requestDetailsOnly } = requestDetails;
  const realEsatetId = requestDetails.real_estates?.id;
    ...
    <main className="ml-0 md:ml-2 lg:ml-4 max-w-7xl  min-h-screen">
      ...
      <header className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-1">
          <Link
            href={`/admin/dashboard/requests/real-estate-requests/${realEsatetId}`}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <RealEstateDetails requestDetails={requestDetails} />
      </div>
    </main>
  );
}
